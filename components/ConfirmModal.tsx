...
// Типы пропсов: сообщение, подтверждение и отмена
type ConfirmModalProps = {
  message: string       // вопрос пользователю (например "Удалить товар?")
  onConfirm: () => void // вызывается при нажатии "Удалить"
  onCancel: () => void  // вызывается при нажатии "Отмена" или клике на фон
}

// Модальное окно подтверждения с двумя кнопками (аналог window.confirm но красивый)
export default function ConfirmModal({ message, onConfirm, onCancel }: ConfirmModalProps) {
  return (
    // Полупрозрачный фон — клик закрывает (отмена)
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={onCancel}>
      <div style={{ backgroundColor: '#fff', borderRadius: '16px', padding: '24px', maxWidth: '400px', width: '100%', textAlign: 'center' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          {/* Кнопка подтверждения — оранжевая */}
