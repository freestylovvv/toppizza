    ...
    <>
      ...
      >
        ...
        <div style={{ padding: '0 8px' }}>
          <h3 style={{ fontSize: '22px', fontWeight: '600', margin: '0 0 8px', color: '#000' }}>{product.name}</h3>
          {(product.requiredIngredients || product.removableIngredients) && (
            <p style={{ fontSize: '14px', color: '#6b6b6b', marginBottom: '16px', lineHeight: '1.4' }}>
              {[product.requiredIngredients, product.removableIngredients].filter(Boolean).join(', ')}
            </p>
          )}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '20px', fontWeight: '600', color: '#000' }}>от {selectedVariant?.price || 0} ₽</span>
            <button
              onClick={(e) => {
                e.stopPropagation()
                setShowModal(true)
