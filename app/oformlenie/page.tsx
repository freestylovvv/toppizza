...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    // sanitizeStr убирает HTML-теги чтобы предотвратить XSS при рендере данных из localStorage
    const sanitizeStr = (v: any) => typeof v === 'string' ? v.replace(/[<>"'`]/g, '') : v
    setCart(savedCart.map((item: any) => ({
      ...item,
      name: sanitizeStr(item.name),
      size: sanitizeStr(item.size),
      comboName: sanitizeStr(item.comboName),
